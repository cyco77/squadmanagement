...
require_once(JPATH_SITE.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_squadmanagement'.DIRECTORY_SEPARATOR.'framework'.DIRECTORY_SEPARATOR.'basesquadcategorylisttemplate.php');
...
class accordionadvancedsquadcategorylisttemplate extends basesquadcategorylisttemplate
{
	public function renderTemplate()
	{
		$doc = JFactory::getDocument();

		$cssHTML = JURI::base().'components/com_squadmanagement/templates/squadcategorylist/AccordionAdvanced/style.css';
		$doc->addStyleSheet($cssHTML);
			
		$html = array();
			
		$html[] = '<script>function showDiv(id) {jQuery("#squadlisttemplate_squadcategories"+id).toggle("slow");}</script>';
		...
		{				
			...
			{		
				if (Count($category->squads))
				{					
					$html[] = '<div onclick="showDiv('.$category->id.')" style="width:100%; cursor:pointer;">';
					$html[] = $category->description;
					$html[] = '</div>';
					
					$html[] = '<div id="squadlisttemplate_squadcategories'.$category->id.'" style="display:none;">';
					
					foreach ($category->squads as $squad) 
					{	  
